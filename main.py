...
def _harden_engine_status_for_session(
    verdict: Optional[str],
    has_primary_candidate: bool,
    market_context: Dict[str, Any],
    time_day_gate: Dict[str, Any],
    ...
    """
    Final guard for top-level / candidate-engine session semantics.

    ACTIVE_NOW must never survive when the regular session is closed or when
    the SAFE-FAST fresh-entry window is closed.
    """
    current = verdict or "NO_TRADE"
    ...
    if not market_context.get("is_open"):
        return "PENDING" if current in {"ACTIVE_NOW", "PENDING"} else current
...
async def _build_on_demand_payload(request: OnDemandRequest) -> Dict[str, Any]:
    ...
    engine_status = _harden_engine_status_for_session(
        verdict=summary_payload.get("verdict", "NO_TRADE"),
        has_primary_candidate=bool(primary_candidate),
        market_context=market_context,
        time_day_gate=time_day_gate,
    )
    ...
    summary_payload["verdict"] = engine_status
    ...
    return {
        "ok": True,
        "mode": "on_demand",
        "build_tag": "schema_patch_trigger_scan_and_engine_status_guard_2026_04_08",
